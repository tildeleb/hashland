...
package siphashpg_test

import (
	"testing"
	"time"

	siphash "leb.io/hashland/siphashpg"
	"leb.io/hrff"
)
...
func TestBasic(t *testing.T) {
	//tv := []byte{0x31, 0x0e, 0x0e, 0xdd, 0x47, 0xdb, 0x6f, 0x72}

	//k := []byte{0xFF, 0xEE, 0xDD, 0xCC, 0xBB, 0xAA, 0x99, 0x88, 0x77, 0x66, 0x55, 0x44, 0x33, 0x22, 0x11, 0xFE}
	//fmt.Printf("len(tv64)=%d, len(tv128)=%d\n", len(tv64), len(tv128))
	if len(tv64) != len(tv128) {
		panic("tv not same length")
	}
	in := make([]byte, len(tv64), len(tv64))
	for k := range tv64 {
		in[k] = byte(k)
	}
	...
	for k, v := range tv64 {
		//fmt.Printf("v=%x\n", v)
		//fmt.Printf("k=%d, in[0:k]=%v\n", k, in[0:k])
		ha, _ := siphash.Siphash(in[0:k], seeds, siphash.Crounds, siphash.Drounds, false)
		//fmt.Printf("ha=%x, hb=%x, tv=%x\n", u64tou8le(ha), u64tou8le(hb), v)
		if ha != siphash.U8tou64le(v) {
			t.Logf("ha=0x%08x != tv=0x%08x\n", ha, siphash.U8tou64le(v))
			t.FailNow()
		}
	}
	...
	for k, v := range tv128 {
		//fmt.Printf("v=%x\n", v)
		//fmt.Printf("k=%d, in[0:k]=%v\n", k, in[0:k])
		ha, hb := siphash.Siphash(in[0:k], seeds, siphash.Crounds, siphash.Drounds, true)
		tva := siphash.U8tou64le(v[0:8])
		tvb := siphash.U8tou64le(v[8:])
		//fmt.Printf("ha=%x, hb=%x, tv=%x\n", u64tou8le(ha), u64tou8le(hb), v)
		//fmt.Printf("ha=0x%08x, hb=0x%08x, tv=%x, tva=0x%08x, tvb=0x%08x\n", ha, hb, v, tva, tvb)
		if ha != tva || hb != tvb {
			t.Logf("ha=0x%08x != tva=0x%08x, hb=0x%08x != tvb=0x%08x\n", ha, tva, hb, tvb)
			t.FailNow()
		}
	}
}
...
func bench(b *testing.B, c, d int, dbl bool) {
	b.SetBytes(int64(b.N * 4))
	start := time.Now()
	for i := 0; i < b.N; i++ {
		k := siphash.U64tou8le(uint64(i))
		_, _ = siphash.Siphash(k, seeds, c, d, dbl)
	}
	stop := time.Now()
